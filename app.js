...
const DB_NAME = "CFM_Database";
const STORE_NAME = "cfm_stories_store";
...
async function loadFromStorage() {
    try {
        ...
        const store = tx.objectStore(STORE_NAME);
        const request = store.get("cfm_stories");
        ...
        if (data) {
            ...
            const oldData = localStorage.getItem("cfm_stories");
            if (oldData) {
                try { 
                    stories = JSON.parse(oldData); 
                } catch (e) { 
                    stories = {}; 
                }
                await saveToStorage();
                localStorage.removeItem("cfm_stories");
            }
        }
    } catch (e) {
